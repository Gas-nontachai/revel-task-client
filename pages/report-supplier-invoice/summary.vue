...
const page_key = 'report-supplier-invoice-summary'
const storage = {
...
const report_headers = ref<{
  ...
  { key: 'supplier_origin_id', header: 'รหัสเจ้าหนี้', },
  { key: 'supplier_name', header: 'ชื่อเจ้าหนี้', },
  ...
  { key: 'avg_unit_price', header: 'ราคาต่อหน่วย', optional: true },
  { key: 'avg_shipping_price', header: 'ราคาต่อหน่วยค่าขนส่ง', optional: true },
  { key: 'avg_promotion_price', header: 'ราคาต่อหน่วยโปรโมชั่น 1', optional: true },
  { key: 'avg_other_price', header: 'ราคาต่อหน่วยอื่น ๆ', optional: true },
...
const report_supplier_invoices = ref<{
  ...
  avg_shipping_price: string,
  avg_promotion_price: string,
  avg_other_price: string,
  total_price: string,
}[]>([])
  ...
  try {
    ...
    reports.forEach(item => {
      report_supplier_invoices.value.push({
        ...
        avg_unit_price: decimalFix(item.invoice_supplier_products.reduce((prev, curr) => prev + toFloat(curr.invoice_supplier_list_unit_price), 0)),
        avg_shipping_price: decimalFix(item.invoice_supplier_products.reduce((prev, curr) => prev + toFloat(curr.invoice_supplier_list_unit_shipping_price), 0)),
        avg_promotion_price: decimalFix(item.invoice_supplier_products.reduce((prev, curr) => prev + toFloat(curr.invoice_supplier_list_unit_promotion_price), 0)),
        avg_other_price: decimalFix(item.invoice_supplier_products.reduce((prev, curr) => prev + toFloat(curr.invoice_supplier_list_unit_other_price), 0)),
...
const exportExcel = async () => {
  ...
  const column_style: any = {
    ...
    avg_unit_price: { style: { alignment: { horizontal: 'right' } }, width: 15, },
    avg_shipping_price: { style: { alignment: { horizontal: 'right' } }, width: 15, },
    avg_promotion_price: { style: { alignment: { horizontal: 'right' } }, width: 15, },
    avg_other_price: { style: { alignment: { horizontal: 'right' } }, width: 15, },
  ...
  worksheet.addRow([`รายงานรับสินค้าตาม Supplier (สรุป)`])
  ...
  report_supplier_invoices.value.forEach((report_supplier_invoice, idx) => {
    const {
      supplier,
      supplier_origin_ids,
      total_invoice,
    ...
    const row: any = {
      idx: idx + 1,
      supplier_origin_id: supplier_origin_ids.join(', '),
...
const exportPDF = async () => {
  ...
  const column_style: any = {
    idx: { width: 80 },
    supplier_origin_id: { width: 180, },
    supplier_name: { width: 300, },
    ...
    avg_shipping_price: { width: 120, },
    avg_promotion_price: { width: 120, },
    avg_other_price: { width: 120, },
  ...
  report_supplier_invoices.value.forEach((report_supplier_invoice, idx) => {
    ...
    const row: any = {
      ...
      supplier_origin_id: `<td>${supplier_origin_ids.join(', ')}</td>`,
      ...
      avg_shipping_price: `<td align="right">${avg_shipping_price}</td>`,
      avg_promotion_price: `<td align="right">${avg_promotion_price}</td>`,
      avg_other_price: `<td align="right">${avg_other_price}</td>`,
  ...
  exportFrom.action = `${publicCtx.apiExportUrl}?export=report-supplier-invoice`
  exportFrom.submit()
};
</script>
...
<template>
  <v-card elevation="10" class="pa-4 withbg" color="grey-lighten-3">
    <v-card-title class="d-flex justify-space-between align-center gap-2">
      <div>รายงานรับสินค้าตาม Supplier <span class="text-error ml-1">(สรุป)</span></div>
    ...
    <v-card-text style="min-height: 500px;">
      ...
      <v-tabs color="primary">
        <v-tab :to="{ path: `/report-supplier-invoice`, query: route_query, }">ละเอียด</v-tab>
        <v-tab active :to="{ path: `/report-supplier-invoice/summary`, query: route_query, }">สรุป</v-tab>
      </v-tabs>
      ...
      <div class="overflow-auto" style="max-height: calc(100vh - 370px); min-height: 50vh;">
        <table class="table table-bordered table-hover" aria-describedby="report"
          :style="{ minWidth: `${(120 * selected_columns.length) + 720}px`, }">
          <thead>
            <tr>
              <th v-if="selected_columns.includes('idx')" style="width: 60px;">ลำดับ</th>
              <th style="width: 140px;">รหัสเจ้าหนี้</th>
              <th>ชื่อเจ้าหนี้</th>
              <th>รายละเอียด</th>
              ...
              <th v-if="selected_columns.includes('deweight_compensation')" style="width: 130px;">น้ำหนักหักชดเชย</th>
              <th v-if="selected_columns.includes('net_weight')" style="width: 120px;">น้ำหนักสุทธิ</th>
              <th v-if="selected_columns.includes('avg_unit_price')" style="width: 120px;">ราคาปกติ</th>
              <th v-if="selected_columns.includes('avg_shipping_price')" style="width: 130px;">ราคาค่าขนส่ง</th>
              <th v-if="selected_columns.includes('avg_promotion_price')" style="width: 130px;">ราคาโปรโมชั่น 1</th>
              <th v-if="selected_columns.includes('avg_other_price')" style="width: 120px;">ราคาอื่น ๆ</th>
          ...
          <tbody>
            ...
            <template v-else>
              <template v-for="(report_supplier_invoice, idx) in report_supplier_invoices">
                <tr>
                  <td v-if="selected_columns.includes('idx')" class="text-center">{{ idx + 1 }}</td>
                  <td>{{ report_supplier_invoice.supplier_origin_ids.join(', ') }}</td>
                  ...
                  <td v-if="selected_columns.includes('avg_unit_price')" class="text-right">
                    {{ report_supplier_invoice.avg_unit_price }}
                  </td>
                  <td v-if="selected_columns.includes('avg_shipping_price')" class="text-right">
                    {{ report_supplier_invoice.avg_shipping_price }}
                  </td>
                  <td v-if="selected_columns.includes('avg_promotion_price')" class="text-right">
                    {{ report_supplier_invoice.avg_promotion_price }}
                  </td>
                  <td v-if="selected_columns.includes('avg_other_price')" class="text-right">
                    {{ report_supplier_invoice.avg_other_price }}
                  </td>
