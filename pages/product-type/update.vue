...
const { getProductTypeByID, updateProductTypeBy } = useProductType();
...
async function onSubmit() {
  ...
  try {
    ...
    await updateProductTypeBy(product_type.value);
...
function validateForm(): boolean {
  if (!(product_type.value.product_type_id && product_type.value.product_type_name)) {
      ...
      <v-card-text class="py-4">
        <v-row>
          <v-col cols="12" sm="6" lg="4">
            ...
            <v-text-field v-model="product_type.product_type_id" density="compact" disabled
