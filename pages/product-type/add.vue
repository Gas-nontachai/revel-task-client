...
const { generateProductTypeID, insertProductType, } = useProductType();
...
onMounted(async () => {
  try {
    const id = await generateProductTypeID()

    product_type.value.product_type_id = id
...
async function onSubmit() {
  ...
  try {
    ...
    await insertProductType(product_type.value);
...
<template>
  ...
  <v-card elevation="10" class="pa-4 withbg">
      ...
      <v-card-text class="py-4">
        <v-row>
          <v-col cols="12" sm="6" lg="4">
            <v-label class="mb-2">รหัสประเภทสินค้า<span class="text-error ml-1">*</span></v-label>
            <v-text-field v-model="product_type.product_type_id" density="compact" :rules="[rules.required]"
              variant="outlined"></v-text-field>
          </v-col>
          <v-col cols="12" sm="6" lg="4">
            <v-label class="mb-2">ชื่อประเภทสินค้า<span class="text-error ml-1">*</span></v-label>
            <v-text-field v-model="product_type.product_type_name" :rules="[rules.required]" density="compact"
              variant="outlined"></v-text-field>
          </v-col>
        </v-row>
        <v-row>
          <v-col cols="12" sm="8" lg="8">
            ...
            <v-textarea v-model="product_type.product_type_detail" density="compact" variant="outlined"></v-textarea>
          </v-col>
        </v-row>
      </v-card-text>
    </template>
    <v-card-text class="d-flex flex-wrap justify-center gap-2">
      ...
      <v-btn color="muted" :to="'/product-type'">ยกเลิก</v-btn>
    </v-card-text>
  </v-card>
</template>