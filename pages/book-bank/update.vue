...
const { getBookBankByID, updateBookBankBy, } = useBookBank();
...
onMounted(async () => {
  try {
    ...
    book_bank_branchs.value = await getBookBankBranchBy({ match: { book_bank_id: query.id }, options: ['branch'], }).then(res => res.docs)
          ...
          <v-col cols="12" md="8">
            <v-row>
              <v-col cols="12" sm="6">
                ...
                <v-text-field v-model="book_bank.book_bank_id" density="compact" disabled
