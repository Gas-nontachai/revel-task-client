...
onMounted(async () => {
  try {
    ...
    license.value.license_all_branch = !!license.value.license_all_branch
    ...
    notify_groups.value.forEach(item => {
      item.all_email_active = item.license_notifys.length == item.license_notifys.filter(val => val.is_email_active).length
      item.all_notify_active = item.license_notifys.length == item.license_notifys.filter(val => val.is_notify_active).length
    })
