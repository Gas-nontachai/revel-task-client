...
const page_key = 'report-product-invoice'
const storage = {
...
const report_headers = ref<{
  ...
  { key: 'supplier_origin_id', header: 'รหัสสินค้า/รหัสเจ้าหนี้', },
  ...
  { key: 'supplier_name', header: 'รายละเอียด', },
  { key: 'branch_name', header: 'บริษัท/สาขา', optional: true },
  ...
  { key: 'invoice_supplier_list_unit_price', header: 'ราคาต่อหน่วย', optional: true },
  { key: 'invoice_supplier_list_price', header: 'จำนวนเงิน', optional: true },
  { key: 'invoice_supplier_list_remark', header: 'หมายเหตุ', optional: true },
])
  ...
  try {
    ...
    reports.forEach(item => {
      item.invoice_supplier_products.forEach(ivp => {
        ...
        ivp.company = companys.value.find(val => val.company_id === ivp.company_id)
        ivp.branch = branchs.value.find(val => val.branch_id === ivp.branch_id)

        ivp.invoice_supplier_date = formatDate(ivp.invoice_supplier_date)
        ivp.invoice_supplier_list_unit_price += toFloat(invoice_supplier_list_unit_shipping_price) + toFloat(invoice_supplier_list_unit_promotion_price) + toFloat(invoice_supplier_list_unit_other_price)
...
const exportExcel = async () => {
  ...
  const column_style: any = {
    ...
    invoice_supplier_list_remark: { width: 35, },
  }
  ...
  worksheet.addRow([`รายงานรับสินค้าตามสินค้า (ละเอียด)`])
  ...
  report_product_invoices.value.forEach((report_product_invoice) => {
    const {
      product,
      product_origin_ids,
    ...
    addRow([
      ...Array.from({ length: product_pre_space }, () => ''),
      `${product_origin_ids.join(', ')} - ${product.product_name}`,
    ])

    worksheet.mergeCells(row_curr_idx, product_pre_space + 1, row_curr_idx, product_pre_space + 4)
    ...
    invoice_supplier_products.forEach((ivp, idx) => {
      ...
      const row: any = {
        idx: idx + 1,
        supplier_origin_id: supplier?.supplier_origin_id,
        invoice_supplier_date,
        invoice_supplier_id,
        invoice_supplier_origin_id,
...
const exportPDF = async () => {
  ...
  const column_style: any = {
    ...
    invoice_supplier_list_remark: { width: 100, },
  }
  ...
  report_product_invoices.value.forEach((report_product_invoice) => {
    ...
    exportData.data.push([
      ...
      `<td colSpan="3">${product_origin_ids.join(', ')} - ${product.product_name}</td>`,
      ...Array.from({
        length: selected_columns.value.filter(column => [
          'invoice_supplier_id',
          'branch_name',
    ...
    invoice_supplier_products.forEach((ivp, idx) => {
      ...
      const row: any = {
        ...
        supplier_origin_id: `<td>${supplier?.supplier_origin_id}</td>`,
        ...
        supplier_name: `<td>${supplier?.supplier_name}</td>`,
        branch_name: `<td>${branch?.branch_name}</td>`,
        ...
        invoice_supplier_list_remark: `<td>${invoice_supplier_list_remark}</td>`,
      }
    ...
    selected_columns.value.includes('invoice_supplier_list_weight') && row_summarys.push(`<td align="right">${total_weight || "-"}</td>`)
    selected_columns.value.includes('invoice_supplier_list_deweight_quality') && row_summarys.push(`<td align="right">${deweight_quality || "-"}</td>`)
    selected_columns.value.includes('invoice_supplier_list_deweight_percent') && row_summarys.push(`<td align="right">${deweight_percent || "-"}</td>`)
    selected_columns.value.includes('invoice_supplier_list_deweight_compensation') && row_summarys.push(`<td align="right">${deweight_compensation || "-"}</td>`)
    selected_columns.value.includes('invoice_supplier_list_net_weight') && row_summarys.push(`<td align="right">${net_weight || "-"}</td>`)
    selected_columns.value.includes('invoice_supplier_list_unit_price') && row_summarys.push(`<td></td>`)
    selected_columns.value.includes('invoice_supplier_list_price') && row_summarys.push(`<td align="right">${total_price || "-"}</td>`)
    selected_columns.value.includes('invoice_supplier_list_remark') && row_summarys.push(`<td></td>`)
...
<template>
  <v-card elevation="10" class="pa-4 withbg" color="grey-lighten-3">
    <v-card-title class="d-flex justify-space-between align-center gap-2">
      <div>รายงานรับสินค้าตามสินค้า <span class="text-error ml-1">(ละเอียด)</span></div>
    ...
    <v-card-text style="min-height: 500px;">
      ...
      <v-tabs color="primary">
        <v-tab active :to="{ path: `/report-product-invoice`, query: route_query, }">ละเอียด</v-tab>
        <v-tab :to="{ path: `/report-product-invoice/summary`, query: route_query, }">สรุป</v-tab>
      </v-tabs>
      ...
      <div class="overflow-auto" style="max-height: calc(100vh - 370px); min-height: 50vh;">
        <table class="table table-bordered table-hover" aria-describedby="report"
          :style="{ minWidth: `${(165 * selected_columns.length) + 720}px`, }">
          <thead>
            <tr>
              <th v-if="selected_columns.includes('idx')" style="width: 60px;">ลำดับ</th>
              <th style="width: 200px;">รหัสสินค้า/รหัสเจ้าหนี้</th>
              ...
              <th v-if="selected_columns.includes('branch_name')" style="width: 400px;">บริษัท/สาขา</th>
              ...
              <th v-if="selected_columns.includes('invoice_supplier_list_remark')">หมายเหตุ</th>
            </tr>
          </thead>
          <tbody>
            ...
            <template v-else>
              <template v-for="(report_product_invoice) in report_product_invoices">
                <tr>
                  <td v-if="selected_columns.includes('idx')"></td>
                  <td colspan="3">
                    {{ report_product_invoice.product_origin_ids.join(', ') }}
                    - {{ report_product_invoice.product.product_name }}
                  </td>
                ...
                <template v-for="(ivp, idx) in report_product_invoice.invoice_supplier_products">
                  <tr>
                    <td v-if="selected_columns.includes('idx')" class="text-center">{{ idx + 1 }}</td>
                    <td>{{ ivp.supplier?.supplier_origin_id }}</td>
                    <td class="text-center">{{ ivp.invoice_supplier_date }}</td>
                    <td v-if="selected_columns.includes('invoice_supplier_id')">{{ ivp.invoice_supplier_id }}</td>
                    <td>{{ ivp.invoice_supplier_origin_id }}</td>
                    <td>{{ ivp.supplier?.supplier_name }}</td>
                    <td v-if="selected_columns.includes('branch_name')">{{ ivp.branch?.branch_name }}</td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_unit_name')">
                      {{ ivp.invoice_supplier_list_unit_name }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_qty')" class="text-right">
                      {{ ivp.invoice_supplier_list_qty }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_weight')" class="text-right">
                      {{ ivp.invoice_supplier_list_weight }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_deweight_quality')" class="text-right">
                      {{ ivp.invoice_supplier_list_deweight_quality }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_deweight_percent')" class="text-right">
                      {{ ivp.invoice_supplier_list_deweight_percent }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_deweight_compensation')"
                      class="text-right">
                      {{ ivp.invoice_supplier_list_deweight_compensation }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_net_weight')" class="text-right">
                      {{ ivp.invoice_supplier_list_net_weight }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_unit_price')" class="text-right">
                      {{ ivp.invoice_supplier_list_unit_price }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_price')" class="text-right">
                      {{ ivp.invoice_supplier_list_price }}
                    </td>
                    <td v-if="selected_columns.includes('invoice_supplier_list_remark')">
                      {{ ivp.invoice_supplier_list_remark }}
                    </td>
                  </tr>
                </template>
                <tr class="bg-lightsuccess">
                  ...
                  <td v-if="selected_columns.includes('branch_name')"></td>
                  ...
                  <td v-if="selected_columns.includes('invoice_supplier_list_unit_price')"></td>
                  <td v-if="selected_columns.includes('invoice_supplier_list_price')" class="text-right">
                    {{ report_product_invoice.total_price }}
                  </td>
                  <td v-if="selected_columns.includes('invoice_supplier_list_remark')"></td>
                </tr>
              </template>
            </template>
          </tbody>
