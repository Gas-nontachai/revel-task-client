...
const { getInvoiceSupplierByID, updateInvoiceSupplierBy, } = useInvoiceSupplier();
...
onMounted(async () => {
  try {
    ...
    branchs.value = await getBranchBy({ match: { company_id: invoice_supplier.value.company_id } }).then(res => res.docs)
    products.value = await getProductBy({ match: { branch_id: invoice_supplier.value.branch_id } }).then(res => res.docs)
    suppliers.value = await getSupplierBy({ match: { branch_id: invoice_supplier.value.branch_id } }).then(res => res.docs)

    invoice_supplier.value.invoice_supplier_date = formatDate(invoice_supplier.value.invoice_supplier_date, 'yyyy-MM-dd')
          ...
          <v-col cols="12" md="12">
            <v-row>
              <v-col cols="12" sm="4" lg="3">
                <v-label class="mb-2">รหัสบันทึกรับ<span class="text-error ml-1">*</span></v-label>
                <v-text-field v-model="invoice_supplier.invoice_supplier_id" density="compact" :rules="[rules.required]"
